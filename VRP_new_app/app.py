...
def main():
    ...
    # Reset file pointer and read for validation
    uploaded_file.seek(0)
    df = pd.read_csv(uploaded_file)
    
    # Validate postcodes
    unique_pcs = pd.concat([df["COLLECTION POST CODE"], df["DELIVER POST CODE"]]).dropna().unique()
    invalid_pcs = [pc for pc in unique_pcs if not mileage.is_valid_postcode(pc)]
    if invalid_pcs:
        st.error(f"Invalid postcodes found: {', '.join(invalid_pcs)}")
        return
    missing_pcs = [pc for pc in unique_pcs if mileage.is_valid_postcode(pc) and not mileage.check_postcode_in_db(pc)]
    if missing_pcs:
        st.error(f"Postcodes not found in database: {', '.join(missing_pcs)}")
        return
